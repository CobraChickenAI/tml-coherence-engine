"""EdgesScreen — confirms exceptions and overrides within Capabilities."""
...
def _exception_assertions(capabilities: list[Capability]) -> list[dict]:
    ...
    for cap in capabilities:
        for exc in cap.exceptions:
            assertions.append(
                {
                    "text": (
                        f"Exception in '{cap.name}': When {exc.trigger}, "
                        f"you {exc.override_description}. "
                        f"Reason: {exc.reason}"
                    ),
...
class EdgesScreen(Screen):
    ...
    def __init__(self, capabilities: list[Capability], **kwargs) -> None:
        ...
        self._assertions = _exception_assertions(capabilities)
    ...
    def _skip(self) -> None:
        """Skip to next screen when there are no assertions."""
    ...
    def _show_current(self) -> None:
        ...
        self.query_one("#counter", Static).update(
            f"Exception {self._current_index + 1} of {len(self._assertions)}"
        )
    ...
    async def _record_provenance(self, action: str, corrected_text: str | None = None) -> None:
        ...
        # Attach to the capability (exceptions are sub-components)
        cap.confirmation = record
        entry = make_provenance_entry(
            scope_id=cap.scope_id,
            primitive_id=cap.id,
            primitive_type="capability.exception",
            action=action,
            actor=actor,
        ...
        # Persist parent capability update (exceptions are sub-components)
