...
_DEFAULT_TEMPLATE = (
    Path(__file__).parent.parent.parent.parent / "templates" / "web_scrape" / "default.yaml"
)

_DEFAULT_PRIMARY_SELECTORS = ["main", "article", ".content", "#content"]
_DEFAULT_STRIP_SELECTORS = ["nav", "footer", "header", ".sidebar", "script", "style"]
...
class ScrapeConfig(BaseModel):
    ...
    primary_selectors: list[str] = _DEFAULT_PRIMARY_SELECTORS.copy()
    strip_selectors: list[str] = _DEFAULT_STRIP_SELECTORS.copy()

    @classmethod
    def from_dict(cls, config: dict) -> ScrapeConfig:
        """Parse from the nested YAML dictionary format."""
        ...
        return cls(
            ...
            respect_robots=target.get("respect_robots", True),
            include_patterns=config.get("include_patterns", []),
            exclude_patterns=config.get("exclude_patterns", []),
            primary_selectors=selectors.get("primary", _DEFAULT_PRIMARY_SELECTORS.copy()),
            strip_selectors=selectors.get("strip", _DEFAULT_STRIP_SELECTORS.copy()),
        )
    ...
    def default(cls, base_url: str) -> ScrapeConfig:
        config = cls.from_yaml(_DEFAULT_TEMPLATE) if _DEFAULT_TEMPLATE.exists() else cls()
...
class WebExtractor(BaseExtractor):
    ...
    def _should_crawl(self, url: str) -> bool:
        ...
        if self._config.include_patterns:
            matched = any(fnmatch.fnmatch(path, p) for p in self._config.include_patterns)
            # Also allow the base URL itself
            if not matched and url.rstrip("/") != self._config.base_url.rstrip("/"):
                return False

        return True

    async def _crawl(self, client: httpx.AsyncClient, url: str, depth: int) -> None:
