...
class InterviewEngine:
    ...
    def get_opening_message(
        self, state: InterviewState, prior_extractions: list | None = None
    ) -> str:
    ...
    def build_system_prompt(
        ...
        if state.discovered_primitives:
            prior_context += "\n\nPrior interview phases have discovered:\n" + json.dumps(
                state.discovered_primitives, indent=2
            )
    ...
    def to_extraction_result(self, state: InterviewState) -> RawExtractionResult:
        ...
        conversation_text = "\n\n".join(
            f"[{msg['role'].upper()}]: {msg['content']}" for msg in state.conversation_history
        )
        content_blocks.append(
            ContentBlock(
